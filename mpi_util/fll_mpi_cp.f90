...
! RECPART      In         receiving partition rank
...
!  Send node header
!
   CALL MPI_SEND(IARR, 3, MPI_INTEGER8, RECPART, SENDPART, COMMUNICATOR, IERR ) 
   CALL MPI_SEND(PNODE%LNAME, NAME_LENGTH, MPI_CHARACTER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
...
!
     IF(ASSOCIATED(PNODE%R1))THEN
       CALL MPI_SEND(PNODE%R1, NDIM*NSIZE, MPI_REAL, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%D1))THEN
       CALL MPI_SEND(PNODE%D1, NDIM*NSIZE, MPI_DOUBLE, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%I1))THEN
       CALL MPI_SEND(PNODE%I1, NDIM*NSIZE, MPI_INTEGER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%L1))THEN
       CALL MPI_SEND(PNODE%L1, NDIM*NSIZE, MPI_INTEGER8, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%S1))THEN
       CALL MPI_SEND(PNODE%S1, NDIM*NSIZE*NAME_LENGTH, MPI_CHARACTER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
!
! 2D ARRAYS
!
     IF(ASSOCIATED(PNODE%R2))THEN
       CALL MPI_SEND(PNODE%R2, NDIM*NSIZE, MPI_REAL, RECPART, SENDPART, COMMUNICATOR, IERR ) 
      RETURN     
     END IF
     IF(ASSOCIATED(PNODE%D2))THEN
       CALL MPI_SEND(PNODE%D2, NDIM*NSIZE, MPI_DOUBLE, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%I2))THEN
       CALL MPI_SEND(PNODE%I2, NDIM*NSIZE, MPI_INTEGER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%L2))THEN
       CALL MPI_SEND(PNODE%L2, NDIM*NSIZE, MPI_INTEGER8, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
     IF(ASSOCIATED(PNODE%S2))THEN
       CALL MPI_SEND(PNODE%S2, NDIM*NSIZE*NAME_LENGTH, MPI_CHARACTER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
       RETURN     
     END IF
!
...
!
   ELSE IF(NDIM*NSIZE == 1 )THEN
     SELECT CASE(CODE)
      CASE(1)
       CALL MPI_SEND(PNODE%R0, 1, MPI_REAL, RECPART, SENDPART, COMMUNICATOR, IERR ) 
      CASE(2)
       CALL MPI_SEND(PNODE%D0, 1, MPI_DOUBLE, RECPART, SENDPART, COMMUNICATOR, IERR ) 
      CASE(3)
       CALL MPI_SEND(PNODE%I0, 1, MPI_INTEGER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
      CASE(4)
       CALL MPI_SEND(PNODE%L0, 1, MPI_INTEGER8, RECPART, SENDPART, COMMUNICATOR, IERR ) 
      CASE(5)
       CALL MPI_SEND(PNODE%S0, NAME_LENGTH, MPI_CHARACTER, RECPART, SENDPART, COMMUNICATOR, IERR ) 
     END SELECT
   END IF  
