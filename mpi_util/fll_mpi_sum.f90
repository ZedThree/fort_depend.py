...
!
    VECSCAL: IF(NDIM > 1)THEN
      ...
      IF(PRESENT(R1))THEN
       ...
       IF(ISTAT /= 0)STOP'ERROR ALLOCATING MEMORY ==> fll_mpi_sum ERR:110 '
       TR1 = R1
       CALL MPI_ALLREDUCE(TR1,R1,NDIM,MPI_REAL,MPI_SUM,COMMUNICATOR,IERR)
       DEALLOCATE(TR1, STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR DEALLOCATING MEMORY ==> fll_mpi_sum ERR:114 '
       RETURN 
      END IF

      IF(PRESENT(D1))THEN
       ALLOCATE(TD1(NDIM), STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR ALLOCATING MEMORY ==> fll_mpi_sum ERR:120 '
       TD1 = D1
       CALL MPI_ALLREDUCE(TD1,D1,NDIM,MPI_DOUBLE,MPI_SUM,COMMUNICATOR,IERR)
       DEALLOCATE(TD1, STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR DEALLOCATING MEMORY ==> fll_mpi_sum ERR:124 '
       RETURN 
      END IF

      IF(PRESENT(I1))THEN
       ALLOCATE(TI1(NDIM), STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR ALLOCATING MEMORY ==> fll_mpi_sum ERR:130 '
       TI1 = I1
       CALL MPI_ALLREDUCE(TI1,I1,NDIM,MPI_INTEGER,MPI_SUM,COMMUNICATOR,IERR)
       DEALLOCATE(TI1, STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR DEALLOCATING MEMORY ==> fll_mpi_sum ERR:134 '
       RETURN 
      END IF

      IF(PRESENT(L1))THEN
       ALLOCATE(TL1(NDIM), STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR ALLOCATING MEMORY ==> fll_mpi_sum ERR:140 '
       TL1 = L1
       CALL MPI_ALLREDUCE(TL1,L1,NDIM,MPI_INTEGER8,MPI_SUM,COMMUNICATOR,IERR)
       DEALLOCATE(TL1, STAT = ISTAT)
       IF(ISTAT /= 0)STOP'ERROR DEALLOCATING MEMORY ==> fll_mpi_sum ERR:144 '
       RETURN 
      END IF
!
!  Scalars
