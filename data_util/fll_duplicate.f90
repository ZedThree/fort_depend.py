...
!  NODE IS A FILE NODE
!
    PNEW => FLL_MK(PNODE%LNAME,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,FPAR,ACTION=LOC_ERRMSG)
    IF(.NOT.ASSOCIATED(PNEW))THEN
      WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
      CALL FLL_OUT(LOC_ERRMSG,FPAR)
      FPAR%SUCCESS = .FALSE.
...
!
    DO WHILE(ASSOCIATED(PCURR))
     ...
     PNEW => FLL_MK(PCURR%LNAME,PCURR%LTYPE,PCURR%NDIM,PCURR%NSIZE,FPAR,ACTION=LOC_ERRMSG)
     IF(.NOT.ASSOCIATED(PNEW))THEN
      WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
      CALL FLL_OUT(LOC_ERRMSG,FPAR)
      FPAR%SUCCESS = .FALSE.
...
!  ADD TO PDUPL LIST
!
     OK = FLL_MV(PNEW,PDUPL,FPAR,ACTION=LOC_ERRMSG)
