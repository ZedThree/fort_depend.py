...
!
    ...
    LOGICAL :: OK
    INTEGER(LINT):: POSOLD
    CHARACTER(LEN=*), OPTIONAL :: LOC_ACT
!
!  READ HEADER
!
    POSOLD = POS
    CALL READ_HEADER(IOUNIT,FMT,POS,NAME,LTYPE,NDIM,NSIZE,FPAR_H,LOC_ACT)
    ...
    IF(TRIM(LTYPE) == 'DIR' .OR. TRIM(LTYPE) == 'N')THEN
       PNODE => FLL_MK(NAME,LTYPE,0_LINT,0_LINT,FPAR_H,LOC_ACT)
    ELSE
      PNODE => FLL_MK(NAME,LTYPE,NDIM,NSIZE,FPAR_H,LOC_ACT)
    END IF
...
!   ATTACH TO PNODE
!
        OK = FLL_MV(PNEW,PNODE,FPAR,LOC_ACT)
...
!
      SELECT CASE(FMT)
      CASE('A')
         CALL READ_DATA_ASCII(IOUNIT,PNODE,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,FPAR_H,LOC_ACT)
      CASE('B')
        IF(SCAN /= 'Y')THEN
          CALL READ_DATA_BIN(IOUNIT,PNODE,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,POS,FPAR_H,LOC_ACT)
        ELSE
          IF(PNODE%NSIZE * PNODE%NDIM == 1)THEN
            CALL READ_DATA_BIN(IOUNIT,PNODE,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,POS,FPAR_H,LOC_ACT)            
          ELSE
            POS = POS + GET_NEW_POS(PNODE,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,FPAR_H,LOC_ACT)
          END IF
        END IF
      END SELECT
...
!
   SUBROUTINE READ_HEADER(IOUNIT,FMT,POS,NAME,LTYPE,NDIM,NSIZE,FPAR,LOC_ACT)
!
...
!  READ DATA 
!
    SUBROUTINE READ_DATA_ASCII(IOUNIT,PNODE,LTYPE,NDIM,NSIZE,FPAR,LOC_ACT)
!
...
!  READ DATA 
!
    SUBROUTINE READ_DATA_BIN(IOUNIT,PNODE,LTYPE,NDIM,NSIZE,POS,FPAR,LOC_ACT)
!
...
!           READ(IOUNIT,IOSTAT=IOSTAT, POS=POS)((PNODE%L2(I,J),J=1,NSIZE),I=1,NDIM)
   ...
   FUNCTION GET_NEW_POS(PNODE,LTYPE, NDIM, NSIZE, FPAR,LOC_ACT) RESULT (POS)
!
