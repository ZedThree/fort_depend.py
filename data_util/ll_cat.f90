...
CONTAINS
   ...
   SUBROUTINE LL_CAT(PNODE,IOUNIT,FPAR)
...
!
   ...
   PCHILD => PNODE%PCHILD
   IF(ASSOCIATED(PNODE%PPAR))WRITE(*,*)' NODE HAS A PARENT, NAME IS ', PNODE%PPAR%LNAME
   CALL LL_PRINT(PNODE, IOUNIT, POS, FPAR)
!
! IF NODE HAS CHILDREN PRINT THEM TOO
!
   IF(ASSOCIATED(PCHILD))CALL LL_CAT_RECURSIVE_NODE(PCHILD,IOUNIT,POS,FPAR)

   FPAR%SUCCESS = .TRUE.

   RETURN
   END SUBROUTINE LL_CAT
!
!  DELETE CHID WITH ALL ITS CHILDREN
!
  RECURSIVE SUBROUTINE LL_CAT_RECURSIVE_NODE(PNODE,IOUNIT,POS,FPAR)
...
!
    DO WHILE(ASSOCIATED(PCURR))
       ...
       CALL LL_PRINT(PCURR, IOUNIT, POS, FPAR)

       PNEXT  => PCURR%PNEXT
       PCHILD => PCURR%PCHILD
       IF(ASSOCIATED(PCHILD))THEN
         CALL LL_CAT_RECURSIVE_NODE(PCHILD,IOUNIT,POS,FPAR)
       END IF
  ...
  END SUBROUTINE LL_CAT_RECURSIVE_NODE
!
!  FREE MEMORY FOR NODE
!
  SUBROUTINE LL_PRINT(PNODE, IOUNIT, POS, FPAR)
    USE LL_TYPE_M
    IMPLICIT NONE
!
!   SUBROUTINE REMOVES NODE
