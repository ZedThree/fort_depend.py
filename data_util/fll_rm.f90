...
!
   CALL FLL_STICH(PNODE,FPAR,LOC_ACT)
!
! IF NODE HAS CHILDREN, REMOVE ALL OF THEM
!
   IF(ASSOCIATED(PCHILD))CALL FLL_RM_RECURSIVE_NODE(PCHILD,FPAR,LOC_ACT)

   CALL FLL_DEALLOC_DATA(PNODE,FPAR,LOC_ACT)
!
!  NULLIFY NODE
!
...
!
  RECURSIVE SUBROUTINE FLL_RM_RECURSIVE_NODE(PNODE,FPAR,LOC_ACT)
!
...
!
    DO WHILE(ASSOCIATED(PCURR))
       ...
       IF(ASSOCIATED(PCURR%PCHILD))THEN
         CALL FLL_RM_RECURSIVE_NODE(PCURR%PCHILD,FPAR,LOC_ACT)
       END IF

       IF(TRIM(PCURR%LTYPE) /= 'LINK')THEN

         CALL  FLL_DEALLOC_DATA(PCURR,FPAR,LOC_ACT)
...
!
  SUBROUTINE FLL_DEALLOC_DATA(PNODE,FPAR,LOC_ACT)
!
