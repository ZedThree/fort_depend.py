...
!
     PLIST => FLL_READ(FILE,IOUNIT,'B',FPAR,SCAN = 'Y',ACTION=LOC_ACT)
     CALL FLL_CAT(PLIST, 6, .TRUE.,FPAR,ACTION=LOC_ACT)
!
   END IF

   PTMP => FLL_LOCATE(PLIST,NAME,LTYPE,DATADIM,NUMBER,.TRUE.,FPAR,ACTION=LOC_ACT)

   IF(.NOT.ASSOCIATED(PTMP))THEN
     CALL FLL_RM(PLIST, FPAR,ACTION=LOC_ACT)
     PNODE => NULL()
     RETURN
   END IF
