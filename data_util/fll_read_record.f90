...
CONTAINS
  ...
  FUNCTION FLL_READ_RECORD(FILE,IOUNIT,PLIST,NAME,LTYPE,DATADIM,NUMBER,RECURSE,FPAR,ACTION) RESULT(PNODE)
!
...
!
     PLIST => FLL_READ(FILE,IOUNIT,'B',FPAR,SCAN = 'Y',ACTION=LOC_ERRMSG)
     CALL FLL_CAT(PLIST, 6, .TRUE.,FPAR,ACTION=LOC_ERRMSG)
!
   END IF

   PTMP => FLL_LOCATE(PLIST,NAME,LTYPE,DATADIM,NUMBER,.TRUE.,FPAR,ACTION=LOC_ERRMSG)

   IF(.NOT.ASSOCIATED(PTMP))THEN
     CALL FLL_RM(PLIST, FPAR,ACTION=LOC_ERRMSG)
     PNODE => NULL()
     RETURN
   END IF
