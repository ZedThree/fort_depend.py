...
CONTAINS
  ...
  FUNCTION LL_READ(FILE,IOUNIT,FMT,FPAR) RESULT(PNODE)
...
!     write(*,*)FPAR_H%SUCCESS
    ...
    IF(TRIM(LTYPE) == 'DIR' .OR. TRIM(LTYPE) == 'N')THEN
       PNODE => LL_MK(NAME,LTYPE,0_LINT,0_LINT,FPAR_H)
    ELSE
      PNODE => LL_MK(NAME,LTYPE,NDIM,NSIZE,FPAR_H)
    END IF
