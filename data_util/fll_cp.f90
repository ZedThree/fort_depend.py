...
! JUST DUPLICATE NODE
!
         PNEW => FLL_DUPLICATE(PWHAT, FPAR,LOC_ACT)
       ELSE
!
...
!
         IF(.NOT.ASSOCIATED(PWHAT,PWHERE))&
            PNEW => FLL_CP_R(PWHAT,PWHERE,'C',FPAR,LOC_ACT) 
       END IF
   ...
   FUNCTION  FLL_CP_R(PWHAT,PWHERE,MODE,FPAR,LOC_ACT) RESULT(PSOURCETMP)
!
! Description: Module copies PWHAT pointer to PWHERE pointer
!              If PWHERE pointer == NULL, PWHAT is a duplicate 
!              of PWHAT with PWHAT%Ppar == NULL
...
!
     IF(MODE == 'M') CALL FLL_STICH(PWHAT,FPAR,LOC_ACT)
...
!   DELETE TARGET NODE
!
     CALL FLL_RM(PWHERE, FPAR,LOC_ACT)
!
!   STICH THE LIST WHERE THE SOURCE WILL BE TAKEN FROM 
!
     IF(MODE == 'M') CALL FLL_STICH(PSOURCETMP,FPAR,LOC_ACT)
!
!  ADD A NEW NODE
!
