...
!
!
MODULE LL_DUPLICATE_M
CONTAINS

   FUNCTION LL_DUPLICATE(PNODE,FPAR) RESULT(PNEW)
...
!
   IF(ASSOCIATED(PCHILD))THEN
     PNEW => LL_MK(PNODE%LNAME,'DIR',0_LINT,0_LINT,FPAR)
     ...
     CALL LL_DUPLICATE_RECURSIVE_NODE(PCHILD,PNEW,FPAR)
...
!  NODE IS A FILE NODE
!
    PNEW => LL_MK(PNODE%LNAME,PNODE%LTYPE,PNODE%NDIM,PNODE%NSIZE,FPAR)
    IF(.NOT.ASSOCIATED(PNEW))THEN
      WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
      FPAR%SUCCESS = .FALSE.
      PNEW => NULL()
      RETURN
    END IF
    CALL LL_COPPY_NODE_ARRAYS(PNODE, PNEW,FPAR)
   ...
   END FUNCTION LL_DUPLICATE
!
!  DELETE CHID WITH ALL ITS CHILDREN
!
  RECURSIVE SUBROUTINE LL_DUPLICATE_RECURSIVE_NODE(PNODE,PDUPL,FPAR)
...
!
      DO WHILE(ASSOCIATED(PCURR))
       ...
       PNEW => LL_MK(PCURR%LNAME,'DIR',0_LINT,0_LINT,FPAR)
       IF(.NOT.ASSOCIATED(PNEW))THEN
        WRITE(FPAR%MESG,'(A)')' DUPLICATE - error allocating PNEW '
        FPAR%SUCCESS = .FALSE.
        PNEW => NULL()
...
!
  ...
  END SUBROUTINE LL_DUPLICATE_RECURSIVE_NODE
!
!  FREE MEMORY FOR NODE
!
  SUBROUTINE LL_COPPY_NODE_ARRAYS(PNODE,PNEW,FPAR)
    USE LL_TYPE_M
    IMPLICIT NONE
!
!   SUBROUTINE REMOVES NODE
...
!
  ...
  END SUBROUTINE LL_COPPY_NODE_ARRAYS
 

END MODULE LL_DUPLICATE_M
