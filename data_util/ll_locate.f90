...
CONTAINS
   ...
   RECURSIVE FUNCTION LL_LOCATE(PNODE,NAME,NUMBER,LTYPE,RECURSE,FPAR) RESULT(PFIND)
...
!
     IF(RECURSE .AND. ASSOCIATED(PCURR%PCHILD))THEN
       PCHLD => PCURR%PCHILD
       PFIND   => LL_LOCATE(PCHLD,NAME,1_LICOMPIL,LTYPE,RECURSE,FPAR)
...
!
   ...
   RETURN
   END FUNCTION LL_LOCATE


END MODULE LL_LOCATE_M
