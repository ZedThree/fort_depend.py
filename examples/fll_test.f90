...
!  2. Locating the node in PNEW1
   ...
   HUMID => FLL_GETNDATA_D1(PNEW1,'Humidity','D',1_LINT,FPAR)
...
!
   PFIND => NULL()
!
! SWEEP IN THE FIRST SUBDIR TEST_Subdir AND PRINT ALL ITEMS WHICH ARE CONTAIN THERE
!
   PTMP => FLL_LOCATE(PNODE,'TEST_Subdir','*',-1_LINT,1_LINT,.FALSE. ,FPAR)
   DO WHILE(FLL_SWEEP(PTMP, PFIND,'*', '*', -1_LINT, .TRUE., FPAR))
      WRITE(*,*)PFIND%LNAME
   END DO
